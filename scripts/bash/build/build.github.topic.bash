...
if [[ -z "${1:-}" ]] 
then
	printf "\\e[1;7;38;5;202m\\n\\n%s\\n\\e[0m\\n" "GitHub topic must be provided;  See \`~/${RDR##*/}/conf/TNAMES\` for topics that build APKs on device with BuildAPKs!  To build all the topic names contained in this file run \`for i in \$(cat ~/${RDR##*/}/conf/TNAMES) ; do ~/${RDR##*/}/scripts/bash/build/build.github.topics.bash \$i ; done\`.  File \`conf/OAUTH\` has important information should you choose to run this command regarding bandwidth supplied by GitHub. "
	exit 227
fi
export TOPIC="$1"
