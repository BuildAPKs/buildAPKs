...
if [[ -z "${1:-}" ]] 
then
	printf "\\e[1;7;38;5;202m\\n\\n%s\\n\\e[0m\\n" "GitHub username must be provided;  See \`~/${RDR##*/}/conf/ONAMES\` for usernames that build APKs on device with BuildAPKs!  To build all the user names contained in this file run \`for i in \$(cat ~/${RDR##*/}/conf/ONAMES) ; do ~/${RDR##*/}/scripts/bash/build/build.github.bash \$i ; done\`.  File \`~/${RDR##*/}/conf/OAUTH\` has important information should you choose to run this command regarding bandwidth supplied by GitHub. "
	exit 227
fi
export UON="${1%/}"
export UONE="${UON##*/}"
