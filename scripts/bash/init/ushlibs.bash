...
_UFSHLIBS_() { 
	...
	then
		...
		printf "\\e[1;7;38;5;96m%s\\e[0m\\n" "Adding ~/${RDR##*/}/scripts/bash/shlibs and ~/${RDR##*/}/scripts/bash/shlibs/buildAPKs..." ; git submodule add https://github.com/shlibs/shlibs.bash scripts/bash/shlibs ; git submodule add -f https://github.com/shlibs/shlibs.buildAPKs.bash scripts/bash/shlibs/buildAPKs  || printf "\\nCannot add modules ~/%s/scripts/bash/shlibs and ~/%s/scripts/bash/shlibs/buildAPKs: Continuing...\\n\\n" "${RDR##*/}" "${RDR##*/}"
	fi
}
