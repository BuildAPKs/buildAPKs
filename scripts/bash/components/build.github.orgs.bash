	...
	then
		...
		then # https://stackoverflow.com/questions/3685970/check-if-a-bash-array-contains-a-value
			...
			then
				...
				then
 					ISAND="$(curl -u "$OAUT" -i "https://api.github.com/repos/$ONAME/$REPO/git/trees/$COMMIT?recursive=1" -s 2>&1 | head -n 4096 || printf "%s\\n" "Error found; Continuing...")"
				else
 					ISAND="$(curl -i "https://api.github.com/repos/$ONAME/$REPO/git/trees/$COMMIT?recursive=1" -s 2>&1 | head -n 4096 || printf "%s\\n" "Error found; Continuing...")"
				fi
