...
_GSU_() { # Update submodules to latest version. 
	(git submodule update --init --recursive --remote $1 2>/dev/null) || (echo ; echo "Cannot update ~/buildAPKs${1:1}: continuing...") # https://www.tecmint.com/chaining-operators-in-linux-with-practical-examples/
}

_CSBMS_() { # Checks whether submodules exist. 
	SBMI=""
	for loc in "${!SBMS[@]}" 
	do
		if [[ ! -f "${SBMS[$loc]}" ]] 
...
_GSMU_() {	
	...
	(git pull 2>/dev/null && echo "Please wait: loading submodules...") || (echo "Cannot update ~/buildAPKs: continuing...")
	_GSU_ ./docs
