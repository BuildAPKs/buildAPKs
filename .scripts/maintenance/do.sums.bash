...
TIME="$(date +%s)"
([[ ! -z "${MTIME##*[!0-9]*}" ]] && (if [[ $(($TIME - $MTIME)) -gt 43200 ]] ; then git pull --ff-only upstream master ; fi) || git pull --ff-only upstream master) || (printf "%s\\n" "Signal generated at [ ! -z \${num##*[!0-9]*} ]" && git pull --ff-only upstream master) || printf "%s\\n" "Signal generated at git pull: Continuing..." 
